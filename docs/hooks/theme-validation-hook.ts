      ...
      lines.forEach((line, index) => {
        bannedPatterns.forEach((pattern) => {
          ...
          if (match && !line.includes("// eslint-disable")) {
            console.error(
              `\x1b[31m[THEME-VALIDATION-HOOK] Banned hardcoded utility class "${match[0]}" found in ${fullPath}:${index + 1}\x1b[0m`
            )
            console.error(
              `\x1b[33mFix: Replace with a semantic taxonomy variable (e.g., bg-beskar-base, text-forge-molten).\x1b[0m`
            )
            process.exit(1)
          }
        })
...
console.log(
  "\x1b[32mTheme Validation Passed. Semantic variable enforcement successful.\x1b[0m"
)
