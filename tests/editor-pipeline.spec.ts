...
test.describe("Editor Happy Path Pipeline", () => {
  test("should complete onboarding, handoff to editor, auto-save, and export PDF", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      const validMockJwt = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9." + encodeBase64Url(JSON.stringify({
      ...
      localStorage.setItem("auth-storage", JSON.stringify({
        state: { user: mockSession.user, session: mockSession, onboardingCompleted: false }
      }));
      localStorage.setItem("sb-vslbiwubtcynvfytwcbr-auth-token", JSON.stringify(mockSession));
    });

    // 2. Mock Supabase REST API responses to simulate the pipeline
    await page.route("**/rest/v1/data_slates*", async (route) => {
      const headers = route.request().headers();
      const isSingle = headers["accept"]?.includes("application/vnd.pgrst.object");
      const fakeSlate = { id: "87654321-4321-4321-4321-210987654321", user_id: "12345678-1234-1234-1234-123456789012" };
      await route.fulfill({
        status: 200,
    ...
    await page.route("**/rest/v1/slate_sections*", async (route) => {
      ...
      if (route.request().method() === "GET") {
          ...
          body: JSON.stringify(isSingle ? null : [
            {
              ...
              raw_content: { 
                ...
                url: "", 
                summary: "", 
                location: { city: "Mandalore Sector", countryCode: "", region: "" }, 
                profiles: [] 
              }
            }
          ]) 
        ...
        await route.fulfill({ status: 201, contentType: "application/json", body: JSON.stringify(isSingle ? {} : []) });
      }
    });
    ...
    await page.route("**/rest/v1/profiles*", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ id: "12345678-1234-1234-1234-123456789012", full_name: "Din Djarin" }),
      });
    });
    ...
    await expect(page.getByText("Hydrating Forge…")).not.toBeVisible({ timeout: 10000 });
    ...
    await expect(previewArea).toContainText("BOUNTY HUNTER", { ignoreCase: true });
