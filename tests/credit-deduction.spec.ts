...
test.describe("Beskar Credit Deduction", () => {
  async function enterEditorAsGuest(page: Page) {
    ...
    await page.getByText("I'm ready to build my resume inside the Forge editor.").click();
  ...
  test("credit balance decrements by 1 after PDF extraction (guest mode)", async ({ page }) => {
    ...
    const dialogBalanceText = page.locator("text=Available Balance:").locator("..").locator("span").nth(1);
    await expect(dialogBalanceText).toBeVisible({ timeout: 5000 });
    const initialText = await dialogBalanceText.textContent() ?? "";
    const initialBalance = parseInt(initialText.match(/(\d+)\s*Credits/)?.[1] ?? "0", 10);
    ...
    await page.getByRole("button", { name: /Extract Sigil \(-1 Credit\)/i }).click();
    ...
    const finalBalance = parseInt(finalText.match(/(\d+)\s*Credits/)?.[1] ?? "0", 10);
