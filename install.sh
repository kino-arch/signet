...
install_cli() {
    ...
    if [ -n "$CODERABBIT_VERSION" ]; then
        # Pinned version â download from the versioned folder
...
main() {
    ...
    if [ "$CHANNEL" = "nightly" ]; then
        print_warning "Installing from the NIGHTLY channel â these builds may be unstable"
        echo
    fi
