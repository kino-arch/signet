...
export async function getCompanyIntel(
  ...
  searchPromises.push(
    fetch(
      `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(cultureQuery)}&api_key=${apiKey}`
    ).then((res) => res.json())
  )
  ...
  searchPromises.push(
    fetch(
      `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(interviewQuery)}&api_key=${apiKey}`
    ).then((res) => res.json())
  )
  ...
  if (jobTitle) {
    ...
    searchPromises.push(
      fetch(
        `https://serpapi.com/search.json?engine=google_jobs&q=${encodeURIComponent(jobsQuery)}&api_key=${apiKey}`
      ).then((res) => res.json())
    )
  }
