...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!NVIDIA_API_KEY)
      throw new Error("Server configuration error: Missing NVIDIA_API_KEY")
...
Required JSON Structure:
{
  ...
  "core_competencies": { "soft_skills": ["string"], "technical_stack": ["string"] }
    ...
    console.log(
      `[ TELEMETRY ] Initiating Nvidia NIM Ingestor for user ${user.id}...`
    )
    ...
    if (!response.ok) {
      console.error(
        `[ ERROR ] Nvidia API Failed: ${response.status} ${response.statusText}`
      )
    ...
    console.log(
      `[ TELEMETRY ] Datacore extraction successful. Returning validated schema.`
    )
