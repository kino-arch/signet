...
import { Check, Download, ShieldAlert, Sparkles, AlertCircle } from "lucide-react";
...
export function SigilRetrieval({
  ...
  const { resumeData } = useForgeStore();
  ...
  const creditBalance = profile?.token_balance ?? 0;
  ...
  useEffect(() => {
    if (progress < 100 || state !== "extracting" || downloadTriggered.current) return;
  ...
  return (
    <div className="flex min-h-[400px] w-full flex-col items-center justify-center py-4">
        ...
        <div className="relative w-full">
          ...
          >
            ...
            <div className="z-10 space-y-2 text-left">
              ...
              >
                {resumeData.basicInfo.firstName || "Operative"} Dossier
              </h2>
              <p
                id={descriptionId}
            ...
            <AnimatePresence initial={false}>
              {isActive && state === "idle" && (
                ...
                >
                  ...
                  {creditBalance <= 0 ? (
                    <Button
                      onClick={(e) => { e.stopPropagation(); onNoTokens?.(); }}
                      variant="destructive"
