import { RefreshCw } from "lucide-react"

import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Button } from "@/components/ui/button"

interface CustomizationsPanelProps {
  onRefreshRules?: () => void
  onAddGlobalRule?: () => void
  onAddWorkspaceRule?: () => void
  onRefreshWorkflows?: () => void
  onAddGlobalWorkflow?: () => void
  onAddWorkspaceWorkflow?: () => void
  onLearnMore?: () => void
}

export function CustomizationsPanel({
  onRefreshRules,
  onAddGlobalRule,
  onAddWorkspaceRule,
  onRefreshWorkflows,
  onAddGlobalWorkflow,
  onAddWorkspaceWorkflow,
  onLearnMore,
}: CustomizationsPanelProps = {}) {
  return (
    <div className="flex flex-col space-y-4 rounded-lg border bg-background p-6 text-foreground">
      <div className="space-y-1">
        <h2 className="text-2xl font-bold tracking-tight">Customizations</h2>
        <p className="text-sm text-muted-foreground">
          Customize Agent to get a better, more personalized experience.{" "}
          <button
            onClick={onLearnMore}
            className="underline underline-offset-4 hover:text-foreground"
          >
            Learn more.
          </button>
        </p>
      </div>

      <Tabs defaultValue="rules" className="w-full">
        <TabsList
          variant="line"
          className="w-full justify-start border-b border-border/50 pb-0"
        >
          <TabsTrigger value="rules" className="text-sm">
            Rules
          </TabsTrigger>
          <TabsTrigger value="workflows" className="text-sm">
            Workflows
          </TabsTrigger>
        </TabsList>

        <TabsContent value="rules" className="mt-0 space-y-4 pt-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <h3 className="text-base font-semibold">Rules</h3>
              <Button
                onClick={onRefreshRules}
                variant="ghost"
                size="icon"
                className="h-6 w-6 rounded-full text-muted-foreground hover:text-foreground"
              >
                <RefreshCw className="h-3.5 w-3.5" />
                <span className="sr-only">Refresh Rules</span>
              </Button>
            </div>
            <div className="flex items-center space-x-2">
              <Button
                onClick={onAddGlobalRule}
                variant="secondary"
                size="sm"
                className="h-8"
              >
                + Global
              </Button>
              <Button
                onClick={onAddWorkspaceRule}
                variant="secondary"
                size="sm"
                className="h-8"
              >
                + Workspace
              </Button>
            </div>
          </div>
          <p className="text-sm text-muted-foreground">
            Rules help guide the behavior of Agent.
          </p>
        </TabsContent>

        <TabsContent value="workflows" className="mt-0 space-y-4 pt-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <h3 className="text-base font-semibold">Workflows</h3>
              <Button
                onClick={onRefreshWorkflows}
                variant="ghost"
                size="icon"
                className="h-6 w-6 rounded-full text-muted-foreground hover:text-foreground"
              >
                <RefreshCw className="h-3.5 w-3.5" />
                <span className="sr-only">Refresh Workflows</span>
              </Button>
            </div>
            <div className="flex items-center space-x-2">
              <Button
                onClick={onAddGlobalWorkflow}
                variant="secondary"
                size="sm"
                className="h-8"
              >
                + Global
              </Button>
              <Button
                onClick={onAddWorkspaceWorkflow}
                variant="secondary"
                size="sm"
                className="h-8"
              >
                + Workspace
              </Button>
            </div>
          </div>
          <p className="text-sm text-muted-foreground">
            Workflows are saved prompts that Agent can follow. To trigger a
            workflow, type "/" in Agent.
          </p>
        </TabsContent>
      </Tabs>
    </div>
  )
}
