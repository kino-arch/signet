...
const ROLES: RoleOption[] = [
  {
    ...
    subtitle: "The Forge",
    description: "Building systems, interfaces, and digital infrastructure. The technical backbone behind every great product.",
  ...
  },
  {
    ...
    description: "Managing client relationships and frontline communications. Turning every interaction into a long-term partnership.",
  ...
  },
  {
    ...
    description: "Orchestrating teams, resources, and strategic initiatives. Turning vision into execution at every level.",
  ...
  },
  {
    ...
    description: "Charting market pathways and analyzing performance data. The strategic minds that drive growth.",
...
export function OnboardingPage() {
  ...
  useEffect(() => {
    if (preOnboardingStep === 1 && !location) {
      const fetchGeo = async () => {
        try {
          const { data, error } = await supabase.functions.invoke("get-user-geo");
          if (!error && data?.threat_origin && data.threat_origin !== "Unknown") {
  ...
  const handleLaunch = async () => {
    ...
    setError(null);
    try {
      const details = { role, firstName, lastName, email: user?.email || "", phone, location, website };

      // 1. Seed the legacy ForgeStore (backward compat with existing templates)
      initializeWithOnboarding(details);
      ...
      if (slateErr || !slate) throw new Error(slateErr?.message || "Failed to create data slate.");

      // 4b. Seed the basics section from onboarding identity data
      const basicsPayload = {
        name: `${firstName} ${lastName}`.trim(),
        label: role === "forge" ? "Software Engineer" : role === "outpost" ? "Customer Success Manager" : role === "guild" ? "Director of Operations" : role === "navigators" ? "Growth Strategist" : "Professional",
        email: user?.email || "",
        phone,
        url: website,
        summary: "",
                ...
                <div className="rounded-2xl border border-border/50 bg-card/60 p-8 backdrop-blur-xl shadow-xl">
                  ...
                  <div className="mb-6 space-y-2 rounded-lg border border-border/50 bg-muted/30 px-4 py-3 font-mono text-[11px]">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-primary">ESTABLISHED</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={cn("font-semibold", scanStatus === "AUTHORIZED" ? "text-primary" : "animate-pulse text-primary/70")}>
                        {scanStatus}
                      </span>
                    </div>
                  ...
                  >
                    {scanStatus === "AUTHORIZED" ? "Enter the Forge" : "Verifying…"}
                  </Button>
                </div>
              </motion.div>
            )}
              ...
              >
                <Onboarding totalSteps={4} onComplete={handleLaunch}>
                  ...
                  <Onboarding.Step step={1}>
                    ...
                    <ChoiceGroup name="role" onValueChange={setRole} value={role} className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      {ROLES.map((r) => {
                        const Icon = r.icon;
                        return (
                          <ChoiceGroup.Item
                      ...
                      <div className="rounded-xl border border-border/50 bg-card/40 p-6 backdrop-blur-xl space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            <Input id="firstName" required placeholder="Alex" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="h-9 bg-background/60 text-sm" />
                          </div>
                          <div className="space-y-1.5">
                            <Label htmlFor="lastName" className="text-xs font-medium text-muted-foreground">Last Name</Label>
                            <Input id="lastName" required placeholder="Morgan" value={lastName} onChange={(e) => setLastName(e.target.value)} className="h-9 bg-background/60 text-sm" />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <Input id="location" required placeholder="San Francisco, CA" value={location} onChange={(e) => setLocation(e.target.value)} className="h-9 bg-background/60 text-sm" />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Input id="website" placeholder="linkedin.com/in/yourname" value={website} onChange={(e) => setWebsite(e.target.value)} className="h-9 bg-background/60 text-sm" />
                        </div>
                      </div>

                      <Onboarding.Navigation canGoNext={!!(firstName && lastName && location)} nextLabel="Continue to Review" />
                    </div>
                  </Onboarding.Step>
                            ...
                            >
                              ...
                              >
                                <div className={cn(
                                  ...
                                  swearOath ? "border-primary bg-primary text-primary-foreground" : "border-muted-foreground/40"
                                )}>
                                  {swearOath && <CheckCircle2 className="h-2.5 w-2.5 fill-current" />}
                                </div>
                        ...
                        <div className="flex flex-col gap-3">
                          <Button
                            ...
                            className={cn(
                              ...
                              swearOath && !submitting && "shadow-[0_0_16px_color-mix(in_srgb,var(--color-primary)_25%,transparent)]"
                            )}
                          >
